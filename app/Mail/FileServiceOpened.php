...
class FileServiceOpened extends Mailable
{
    ...
    {
        $emailTemplate = EmailTemplate::where('company_id', $this->fileService->user->company->id)->where('label', 'file-service-opened-email')->first(['subject', 'body']);
        if($emailTemplate){
