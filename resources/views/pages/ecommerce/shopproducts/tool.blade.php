      ...
      @foreach ($entries as $i => $entry)
        <tr>
          ...
          <td class="td-actions">
            ...
            <a class="btn btn-icon btn-primary" href="{{ route($route_prefix.'shopproducts.edit', ['shopproduct' => $entry->id]) }}" title="Edit">
              <i data-feather="edit"></i>
            </a>
            @endif
            <a class="btn btn-icon btn-danger" onclick="onDelete(this)" data-id="{{ $entry->id }}" title="Delete"><i data-feather="trash-2"></i></a>
            <form action="{{ route($route_prefix.'shopproducts.destroy', $entry->id) }}" class="delete-form" method="POST" style="display:none">
