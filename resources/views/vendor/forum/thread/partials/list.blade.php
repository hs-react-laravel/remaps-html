<div class="list-group-item {{ $thread->pinned ? 'pinned' : '' }} {{ $thread->locked ? 'locked' : '' }} {{ $thread->trashed() ? 'deleted' : '' }}" :class="{ 'border-primary': selectedThreads.includes({{ $thread->id }}) }">
    <div class="row align-items-center text-center">
        <div class="col-sm text-md-start">
            <span class="lead">
                <a href="{{ route('cf.thread.show', $thread) }}" @if (isset($category))style="color: {{ $category->color }};"@endif>{{ $thread->title }}</a>
            </span>
            <br>
            {{ $thread->authorName }} <span class="text-muted">@include ('vendor.forum.partials.timestamp', ['carbon' => $thread->created_at])</span>

            @if (! isset($category))
                <br>
                <a href="{{ route('cf.category.show', $thread->category) }}" style="color: {{ $thread->category->color }};">{{ $thread->category->title }}</a>
            @endif
        </div>
        ...
        @if ($thread->lastPost)
            <div class="col-sm text-md-end text-muted">
                @php
                    ...
                    $append = "#post-{$lastPost->sequence}";
                @endphp
                <a href="{{ route('cf.thread.show', ['thread' => $thread, 'page' => ceil($lastPost->sequence / $lastPost->getPerPage())]).$append }}">{{ trans('forum::posts.view') }} &raquo;</a>
                <br>
                {{ $thread->lastPost->authorName }}
                <span class="text-muted">@include ('vendor.forum.partials.timestamp', ['carbon' => $thread->lastPost->created_at])</span>
            </div>
        @endif
